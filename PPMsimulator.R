...
PPMsimulator <- function(parameter1,parameter2,minx,maxx,miny,maxy,weight) {
  #setting up window
  ...
  mydata<-as.data.frame(mydata)
  myweights <- c(array(1,length(X.polr)/3),array(weight,length(X.dpp)/3))
  wlrglm <- glm(V3~V1+V2,data=mydata,family="binomial",weights=myweights)
  #Weighted logistic regression using optim
  ...
  #Summary
  mylist<-list(logisticregression = lrfit, optimIPP = fitIPP,glmIPP = IPPglm,optimweighted = wlrfit,glmweighted = wlrglm)
  return(mylist)
