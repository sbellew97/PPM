...
X.back1 <- cbind(array(1,length(sgrid.bio3)),sgrid.bio8,sgrid.bio9,sgrid.bio13,sgrid.bio14)
```
```{r, include=FALSE}
...
```{r}
  ...
  Xtrue <- cbind(array(1,dim(s)[1]*dim(s)[2]),values(s)[,'bio3'],values(s)[,'bio8'],values(s)[,'bio9'],values(s)[,'bio13'],values(s)[,'bio14'])
  X <- Xtrue[,c(1,3,4,5,6)]
  gridsizeratio=sum(!is.na(X))/length(X)
  parameter <- c(5,0.2,0.3,-0.2,0.26,-0.2)
  B <- parameter
  values(s) <- exp(Xtrue%*%B)
  maxlambda <- max(values(s),na.rm=TRUE)
  parameter<-parameter[c(1,3,4,5,6)]
  B<-parameter
  
``` 
We then run N simulations and average over the fits. Spatstat has been used as a comparison, with the covariates normalised and formatted to use in ppm. The misspecified model has bio3 removed but retains the 4 other covariates.
...
tmfitIPPBT=0
tmppmIPP=0
aucfitIPP=matrix(0,N)
aucfitIPPBT=matrix(0,N)
aucwlrfit=matrix(0,N)
auclrbadd=matrix(0,N)
aucppmIPP=matrix(0,N)
...
covlist<-list(a=cov8,b=cov9,c=cov13,d=cov14)
 w <- as.owin(cov3)
 
 #negative Poisson likelihood function (grid only)
...
#fit Baddeley Regression
  ...
  lrbadd <- glm(V5~sgrid.bio8+sgrid.bio9+sgrid.bio13+sgrid.bio14+offset(-offst),data=mydata,family="binomial")
   tmlrbadd=tmlrbadd+Sys.time()-tm
  totlrbadd[i,]=coefficients(lrbadd)
  maxlllrbadd[i,]=logLik(lrbadd)
  int <- predict(lrbadd, type = "intensity")
auclrbadd[i] <- spatstat::auc(X=s.loc.ipp,covariate=int)
...
ppmIPP<-ppm(datadummy,trend=~sgrid.bio8+sgrid.bio9+sgrid.bio13+sgrid.bio14,data=mydata2,method='mpl'
)
tmppmIPP=tmppmIPP+Sys.time()-tm
totppmIPP[i,]=coefficients(ppmIPP)
maxllppmIPP[i,]=logLik(ppmIPP)-sum(log(1:N.ipp))
int <- predict(ppmIPP, type = "intensity")
aucppmIPP[i] <- spatstat::auc(X=s.loc.ipp,covariate=int)
...
boxfitIPP <- list(stats=matrix(c(sdata[[1]],quantile(totfitIPP[,3],0.025),sdata[[3]],quantile(totfitIPP[,3],0.975),sdata[[5]]),5,1),n=10)
sdata <- summary(totwlrfit[,3])[c(1,2,4,5,6)]
boxwlrfit <- list(stats=matrix(c(sdata[[1]],quantile(totwlrfit[,3],0.025),sdata[[3]],quantile(totwlrfit[,3],0.975),sdata[[5]]),5,1),n=10)
sdata <- summary(totlrbadd[,3])[c(1,2,4,5,6)]
boxlrbadd <- list(stats=matrix(c(sdata[[1]],quantile(totlrbadd[,3],0.025),sdata[[3]],quantile(totlrbadd[,3],0.975),sdata[[5]]),5,1),n=10)
sdata <- summary(totppmIPP[,3])[c(1,2,4,5,6)]
boxppmIPP <- list(stats=matrix(c(sdata[[1]],quantile(totppmIPP[,3],0.025),sdata[[3]],quantile(totppmIPP[,3],0.975),sdata[[5]]),5,1),n=10)
...
#abline(h=B[4],col="Red")

```
```{r}
summary.IPP <- matrix(0,5,6)
...
rmseppmIPP=0
for (i in 1:N){
rmse1 <- rmse(B,totfitIPP[i,])
rmsefitIPP=rmsefitIPP+rmse1^2
rmse1 <- rmse(B,totfitIPPBT[i,])
rmsefitIPPBT=rmsefitIPPBT+rmse1^2
rmse1 <- rmse(B,totwlrfit[i,])
rmsewlrfit=rmsewlrfit+rmse1^2
rmse1 <- rmse(B,totlrbadd[i,])
rmselrbadd=rmselrbadd+rmse1^2
rmse1 <- rmse(B,totppmIPP[i,])
rmseppmIPP=rmseppmIPP+rmse1^2
}
rmsefitIPP=sqrt(rmsefitIPP)
rmsefitIPPBT=sqrt(rmsefitIPPBT)
rmsewlrfit=sqrt(rmsewlrfit)
rmselrbadd=sqrt(rmselrbadd)
rmseppmIPP=sqrt(rmseppmIPP)
